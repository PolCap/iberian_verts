...
ROUSS.pred = function(mu, theta, betasq, tausq, Yobs,Tvec,
                      name,
                      plot.it="FALSE") {
  qp1 = length(Yobs)
  q = qp1-1
  ...
  if(plot.it=="TRUE") {
    ...
    X11()
    plot(Tvec,exp(Yobs),xlab="time",ylab="abundance",lty=1,
  ...
  return(data.frame(name, Tvec,Predict.t, N=Yobs))
}
...
# 15. EGSS predictions  --------------------------------------------------------
# Function to predict abundances from observed values using the EGSS model with
# a Kalman filter. 
# mu = mu estimate from the model
# ssq = the sigma value from the model
# tsq = the tau value from the model
# xo = 
# N = observed abundance
# Tvec = time interval

EGSS.pred = function(mu, ssq, tsq, xo, Yobs, Tvec, name) {
  T.t = Tvec - Tvec[1] # Time starts at zero
  
  Y.t <-  Yobs # The observations
  ...
  #  Choose ML estimates here for calculating model values
  m[1] <-  xo       
  #  Initial mean of Y(t)
  v[1] <-  tsq 
  ...
  out <- data.frame(name, Tvec, Predict.t, N=Yobs)
  return(out)
}

# PART III):  PBLR function --------------------------------

# 16. Function to do a PBLRT between the EGSS model (null)
...
PBLRT.ddpcont = function(B=10,Yobs,Tvec,alpha=0.05,plot.PBLR=TRUE) {
  ...
  guess.egss = tryCatch(init.egss(Yobs=Yobs,Tvec=Tvec), 
                        error=function(err) NA)
  egssml = tryCatch(EGSS.ML(Yobs=Yobs,Tvec=Tvec,parms.guess=guess.egss), 
                    error=function(err) NA)
  lnL.Ho = tryCatch(egssml$lnL.hat, 
                    error=function(err) NA)
  #  Estimation under the alternative H1:  the OUSS model
  guess.rouss  = tryCatch(guess.calc(Yobs=Yobs,Tvec=Tvec), 
                          error=function(err) NA)
  roussml = tryCatch(ROUSS.ML(Yobs=Yobs,Tvec=Tvec,parms.guess=guess.rouss), 
                     error=function(err) NA)
  lnL.H1 = tryCatch(roussml$lnL.hat, 
                    error=function(err) NA)
  ...
  if(is.na(egssml[1])){
    out = data.frame(egss.mu=NA,
               egss.sigma=NA,
               egss.tau=NA,
               egss.xo=NA,
               egss.AIC=NA)}else{
                 out = data.frame(egss.mu=egssml$mles[[1]],
                                  egss.sigma=egssml$mles[[2]],
                                  egss.tau=egssml$mles[[3]],
                                  egss.xo=egssml$mles[[4]],
                                  egss.AIC=egssml$AIC)  
               }
  if(is.na(roussml[1])){
    out$ouss.mu=NA
    ...
    out$ouss.mu=roussml$mles[[1]]
    out$ouss.theta=roussml$mles[[2]]
    out$ouss.beta=roussml$mles[[3]]
    out$ouss.tau=roussml$mles[[4]]
    out$ouss.AIC=roussml$AIC
  }

             # Lam.obs=Lam.obs,
             # Lam.vec=Lam.vec,
             # pvalue=pval,Decision.rule=Decision.rule)
  #   if(plot.PBLR==TRUE) {
